  ...
  function snapshot() {
    ...
    if (!enemies || enemies.total === 0) {
      ...
      shown.forEach((e, i) => {
        ...
        // her eyes, in words: species + state + range + direction. No tiers,
        // no colors, no pixels, no hp, no prices — code aims by numbers, she thinks in words.
        lines.push(`  #${i + 1}: ${sp}${e.hostile ? 'HOSTILE, will bite' : 'calm, milling around'} — ${rangeWord(e.dist)} to the ${dirName(e.dx, e.dy)}.`);
      });
